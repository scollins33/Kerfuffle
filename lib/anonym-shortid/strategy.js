 ...
 * of the form:
 *
 * {shortid: "anonymous_710b962e-041c-11e1-9234-0123456789ab"}
 *
 * Applications can supply a `verify` callback which accepts `req` and `shortid`
...
Strategy.prototype.authenticate = function(req) {
    ...
    let shortid = "player_" + shortid.generate();
    let self = this;
    ...
    if (this._verify) {
        try {
            this._verify(req, shortid, verified);
        } catch (ex) {
            return self.error(ex);
        }
    } else verified(null, {shortid})
};
